...
from sklearn.metrics import accuracy_score, precision_score, recall_score, roc_auc_score, f1_score, confusion_matrix, average_precision_score
...
def main():
    try:
        ...
        try:
            data = load_processed_data(PROJECT_PATH, windows=["FM12"], segments=["red"], splits=["train"])
            if data.empty:
        ...
        out_dir = os.path.join(PROJECT_PATH, 'outputs', 'exploration')
        ...
        plt.ylabel('Actual')
        plt.tight_layout()
        save_path = os.path.join(out_dir, 'confusion_matrix_oldreglog.png')
